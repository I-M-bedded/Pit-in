...
class KDESoftHistogramLoss(nn.Module):
    ...
    def _soft_hist(self, x: torch.Tensor) -> torch.Tensor:
        # Normalize to [0, 1] for histogram centers
        x_min = x.min()
        x_max = x.max()
...
class TDDRPLoss(nn.Module):
    ...
    """
    PHASE_WEIGHTS = {
        1: dict(kd=0.3, ssl=1.0, hist=1.0, smooth=1.0, tv=0.1, orth=0.1),
    ...
    def forward(
        ...
        losses["tv"] = self.lambda_tv * sum(
            self.tv_loss(dg) + self.tv_loss(db) for dg, db in zip(delta_gammas, delta_betas)) / n
        losses["orth"] = self.lambda_orth * sum(
            self.orth_loss(ew) for ew in expert_weights_list) / len(expert_weights_list)

        # KD Loss: multi-scale 평균 + L1 (MSE보다 outlier에 안정적)
        if self.lambda_kd > 0 and pose_logits_aug is not None and pose_logits_clean is not None:
            def _l1_recursive(a, c):
                if isinstance(a, torch.Tensor) and isinstance(c, torch.Tensor):
                    ...
                    return sum(_l1_recursive(a[k], c[k]) for k in keys) / max(len(keys), 1)
...
class TDDRPEndToEndPose(nn.Module):
    ...
    Phase 2 개선:
    - Frozen Teacher: 별도 backbone 복사본으로 clean 이미지 처리 (collapse 방지)
    - Task Loss: 외부에서 PoseLoss26과 함께 사용하여 GT anchor 제공
    """
    ...
    def __init__(self, yolo_model, num_experts=3):
        super().__init__()
        self.yolo_model = yolo_model.model.model          # nn.Sequential (backbone+neck+head)
        self.save_list = yolo_model.model.save
        # PoseLoss init용 참조. submodule 등록하면 yolo_model과 파라미터 중복 → state_dict 불일치.
        # object.__setattr__로 nn.Module 등록 우회.
        object.__setattr__(self, '_yolo_detection_model', yolo_model.model)

        self.teacher_backbone = None  # Phase 2에서 frozen teacher 생성
    ...
    def _init_frozen_teacher(self):
        """현재 backbone을 deepcopy하여 frozen teacher 생성 (Phase 2 시작 시 호출)"""
    ...
    def train(self, mode=True):
        """Teacher backbone은 항상 eval 모드 유지 (BN running stats 사용)"""
        super().train(mode)
    ...
    def _run_backbone(self, x, modulate=False, Z=None, P=None, backbone=None):
        """YOLO 순전파. backbone 인자로 teacher/student 선택 가능"""
        if backbone is None:
        ...
        return F_targets, delta_gammas, delta_betas, pose_logits, x
    ...
    def forward(self, I_aug, I_clean=None):
        ...
        # Teacher path: frozen teacher가 있으면 사용, 없으면 현재 backbone (Phase 1)
        if I_clean is not None:
            teacher = self.teacher_backbone if self.teacher_backbone is not None else self.yolo_model
            with torch.no_grad():
                F_cleans, _, _, pose_logits_clean, x_clean = self._run_backbone(
                    I_clean, modulate=False, backbone=teacher)
            outputs["F_cleans"] = F_cleans
            outputs["pose_logits_clean"] = pose_logits_clean
            outputs["x_clean"] = x_clean

        # Student path (aug + FiLM modulation)
        F_primes, delta_gammas, delta_betas, pose_logits_aug, x_aug = self._run_backbone(
